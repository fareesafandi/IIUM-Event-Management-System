<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('events.index') }}" class="text-sm text-indigo-600 hover:text-indigo-800 dark:text-indigo-400">
                ← Back to Events
            </a>
        </div>
    </x-slot>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div class="lg:col-span-2 space-y-6">
                            ...
                            @if($event->poster_image_url)
                                <img src="{{ $event->poster_image_url }}" alt="{{ $event->title }}" class="w-full h-96 object-cover rounded-lg">
                            @else
                            ...
                            <div class="flex items-center justify-between mb-4">
                                ...
                                @if($isManager)
                                    <a href="{{ route('manager.events.edit', $event) }}" class="text-sm text-indigo-600 hover:text-indigo-800">
                                        Edit Event
                                    </a>
                                @endif
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                                <div class="flex items-start">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-gray-900 dark:text-gray-100">{{ $event->date->format('l, F d, Y') }}</p>
                                        <p class="text-gray-900 dark:text-gray-100">{{ date('g:i A', strtotime($event->time)) }}</p>
                                    </div>
                                </div>
                                ...
                                <div class="flex items-start">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-gray-900 dark:text-gray-100">{{ $event->venue }}</p>
                                    </div>
                                </div>
                                ...
                                @if($event->organizer_name)
                                    <div class="flex items-start">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-gray-900 dark:text-gray-100">{{ $event->organizer_name }}</p>
                                            @if($event->organizer_contact)
                                                <p class="text-sm text-gray-600 dark:text-gray-400">{{ $event->organizer_contact }}</p>
                                            @endif
                                            @if($event->organizer_email)
                                                <p class="text-sm text-gray-600 dark:text-gray-400">{{ $event->organizer_email }}</p>
                                            @endif
                                        </div>
                                    </div>
                                @endif
                            ...
                            @if($relatedEvents->count() > 0)
                                <div class="pt-6 border-t border-gray-200 dark:border-gray-700">
                                    <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Related Events</h3>
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        @foreach($relatedEvents as $relatedEvent)
                                            <a href="{{ route('events.show', $relatedEvent) }}" class="block p-4 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                                <h4 class="font-semibold text-gray-900 dark:text-gray-100">{{ $relatedEvent->title }}</h4>
                        ...
                        <div class="lg:col-span-1">
                            <div class="bg-gray-50 dark:bg-gray-900 p-6 rounded-lg sticky top-4">
                                @if($isRegistered)
                                    <div class="mb-4">
                                    ...
                                    @if($isManager)
                                        <a href="{{ route('manager.events.participants', $event) }}" class="block w-full text-center px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-md font-medium transition-colors mb-4">
                                            View Participants
                                        </a>
                                    @endif
                                @else
                                    @auth
                                        @if($event->canAcceptRegistrations())
                                            <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Register for this Event</h3>
                                            <form action="{{ route('registrations.register', $event) }}" method="POST">
                                                ...
                                                <div class="space-y-4">
                                                    ...
                                                    <div>
                                                        <label for="kulliyah" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                                                            Kulliyah *
                                                        </label>
                                                        <input 
                                                            type="text" 
                                                    ...
                                                    <button type="submit" class="w-full px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-md font-medium transition-colors">
                                    ...
                                    @else
                                        <div class="bg-blue-50 border border-blue-200 text-blue-800 px-4 py-3 rounded-lg">
                                            ...
                                            <a href="{{ route('login') }}" class="block w-full text-center px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-md font-medium transition-colors">
                                                Login
                                            </a>
                                        </div>
                                    @endauth
