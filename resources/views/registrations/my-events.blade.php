...
<div class="bg-gray-50 min-h-screen py-12">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6">
            ...
            <p class="mt-2 text-gray-600">View and manage all events you've registered for</p>
        </div>
                    ...
                    <div class="bg-white rounded-lg shadow-sm overflow-hidden hover:shadow-lg transition-shadow">
                        ...
                        <div class="p-6">
                            ...
                            <h3 class="text-xl font-semibold text-gray-900 mb-2">
                                <a href="{{ route('events.show', $event) }}" class="hover:text-teal-600">
                                    {{ $event->title }}
                                </a>
                            </h3>
                            ...
                            <div class="flex gap-2">
                                <a href="{{ route('events.show', $event) }}" class="flex-1 text-center px-4 py-2 bg-teal-500 hover:bg-teal-600 text-white rounded-md text-sm font-medium transition-colors">
                                    View Details
                                </a>
                                @if(!$isPast)
                                    <form action="{{ route('registrations.unregister', $event) }}" method="POST" class="flex-1">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="w-full px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-md text-sm font-medium transition-colors">
                                            Unregister
                                        </button>
                                    </form>
                                @endif
        ...
        @else
            <div class="bg-white rounded-lg shadow-sm p-12 text-center">
                ...
                <h3 class="mt-2 text-lg font-medium text-gray-900">No registered events</h3>
                <p class="mt-1 text-sm text-gray-500">
                    You haven't registered for any events yet.
                </p>
                <div class="mt-6">
                    <a href="{{ route('events.index') }}" class="inline-flex items-center px-4 py-2 bg-teal-500 hover:bg-teal-600 text-white rounded-md font-medium transition-colors">
                        Browse Events
                    </a>
                </div>
            </div>
